...
struct CompilerData {
  ...
  CompilerData(const void *CmdArgs, unsigned CmdArgsLen, const void *ASTBuffer,
    ...
    Reader = new ASTReader(*PP, *PCMCache, Ctx.get(), *PCHContainerRdr, {},
                           /*isysroot=*/"",
    ...
    for (auto &TA : FTSI->TemplateArguments->asArray()) {
      auto HandleTA = [&](const TemplateArgument &TA) {
        ...
        if (TA.getAsExpr()->EvaluateAsConstantExpr(
                Eval, Expr::EvaluateForMangling, *Ctx)) {
          TAIsSaved.push_back(TASK_None);
          return;
        }
                  ...
                  if (NonTypeTemplateParmDecl *NTTP =
                    ...
                    if (IsArray &&
                      ...
                      QualType ArrTy = Ctx->getConstantArrayType(
                          ElemTy, Sz, nullptr, clang::ArrayType::Normal, 0);
...
extern "C"
  ...
  {
    std::lock_guard<sys::Mutex> Guard(IMutex);
