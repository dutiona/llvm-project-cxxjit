...
Address CodeGenFunction::EmitPointerWithAlignment(const Expr *E,
    ...
    case CK_AddressSpaceConversion:
      if (auto PtrTy = CE->getSubExpr()->getType()->getAs<PointerType>()) {
        ...
        if (isa<ExplicitCastExpr>(CE)) {
          ...
          CharUnits Align = getNaturalPointeeTypeAlignment(
  ...
  CharUnits Align =
      getNaturalPointeeTypeAlignment(E->getType(), BaseInfo, TBAAInfo);
  ...
  if (Info.StorageSize != Info.Size) {
    ...
    if (!hasBooleanRepresentation(Dst.getType()))
      SrcVal = Builder.CreateAnd(
          SrcVal, llvm::APInt::getLowBitsSet(Info.StorageSize, Info.Size),
          "bf.value");
    MaskedVal = SrcVal;
    if (Info.Offset)
  ...
  if (const VectorType *VTy = Dst.getType()->getAs<VectorType>()) {
    ...
    if (NumDstElts == NumSrcElts) {
      ...
      Vec = Builder.CreateShuffleVector(
          SrcVal, llvm::UndefValue::get(Vec->getType()), MaskV);
      ...
      llvm::Value *ExtSrcVal = Builder.CreateShuffleVector(
          SrcVal, llvm::UndefValue::get(SrcVal->getType()), ExtMaskV);
      // build identity
      SmallVector<llvm::Constant *, 4> Mask;
...
static Address emitOMPArraySectionBase(CodeGenFunction &CGF, const Expr *Base,
  ...
  if (auto *ASE = dyn_cast<OMPArraySectionExpr>(Base->IgnoreParenImpCasts())) {
    ...
    CharUnits Align =
        CGF.getNaturalTypeAlignment(ElTy, &TypeBaseInfo, &TypeTBAAInfo);
