...
void EmitAssemblyHelper::FinalizeForJIT() {
  ...
  for (auto &F : TheModule->functions())
    for (auto &BB : F)
      for (auto &I : BB)
        if (auto CS = dyn_cast<llvm::CallBase>(&I))
          if (Value *Callee = CS->getCalledOperand())
            if (Function *F =
  ...
  llvm::Value *IRDataLen = llvm::ConstantInt::get(
      JCalls[0]->getArgOperand(5)->getType(), Data.size());
...
void EmitBackendOutput(DiagnosticsEngine &Diags,
                       ...
                       const CodeGenOptions &CGOpts,
                       const clang::TargetOptions &TOpts,
                       const LangOptions &LOpts, StringRef TDesc, Module *M,
                       BackendAction Action,
                       std::unique_ptr<raw_pwrite_stream> OS) {
